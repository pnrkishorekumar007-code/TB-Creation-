      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-panel2 border border-paper/10 overflow-hidden flex items-center justify-center text-xl font-bold uppercase shrink-0">
              {(preview || (user.avatarUrl ? `${API_BASE}${user.avatarUrl}` : null)) ? (
                <img src={preview || `${API_BASE}${user.avatarUrl}`} alt="Avatar preview" className="w-full h-full object-cover" />
              ) : (
                user.name?.[0] || '?'
              )}
            </div>
            <div>
              <label className="inline-block px-4 py-2 bg-accent text-ink text-sm font-semibold rounded-md cursor-pointer hover:opacity-90 transition">
                Choose File
                <input
                  type="file"
                  accept="image/*"
              ...
              {avatar && <p className="text-xs text-muted mt-2 break-all">{avatar.name}</p>}
            </div>
          </div>
        </div>
