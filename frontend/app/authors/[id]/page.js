...
export default function AuthorProfilePage() {
  ...
  if (notFound) return <p className="max-w-6xl mx-auto px-5 py-10 text-muted">This author profile isn't available.</p>;
  if (error) {
    return (
      <div className="max-w-6xl mx-auto px-5 py-10 text-center">
        <p className="text-muted mb-4">{error}</p>
        <button onClick={load} className="px-5 py-3 bg-accent text-ink font-semibold rounded-md">
          Try Again
        </button>
      </div>
    );
